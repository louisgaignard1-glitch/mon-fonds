...
def generate_pdf_factsheet(portfolio_index, portfolio_index_hedged, bench_index, weights, top5, prices_eur, ticker_names):
    ...
    # --- En-tête ---
    elements.append(Paragraph("Factsheet Financière - Performance du Portefeuille", styles["Title"]))
    elements.append(Paragraph(f"Date : {pd.Timestamp.today().strftime('%d/%m/%Y')}", styles["Normal"]))
    elements.append(Spacer(1, 0.25*inch))

    # --- Graphique principal (performance cumulée) ---
    plt.figure(figsize=(8, 4))
    plt.plot(portfolio_index.index, portfolio_index, label="Portfolio", color="blue", linewidth=2)
    plt.plot(bench_index.index, bench_index, label="Benchmark", color="red", linewidth=2)
    plt.plot(portfolio_index_hedged.index, portfolio_index_hedged, label="Portfolio Hedgé", color="green", linestyle="--", linewidth=2)
    ...
    img_buffer.seek(0)
    plt.close()
    elements.append(Paragraph("Performance cumulée", styles["Heading2"]))
    elements.append(Image(img_buffer, width=7*inch, height=3*inch))
    elements.append(Spacer(1, 0.25*inch))

    # --- Top 5 performants ---
    plt.figure(figsize=(6, 3))
    top5.sort_values(ascending=False).plot(kind="bar", color=["#2ecc71"]*5)
    plt.title("Top 5 des actifs performants (1 mois)")
    plt.ylabel("Performance (%)")
    plt.xticks(rotation=45)
    img_top5_buffer = BytesIO()
    plt.savefig(img_top5_buffer, format="png", dpi=300, bbox_inches="tight")
    img_top5_buffer.seek(0)
    plt.close()
    elements.append(Paragraph("Top 5 des actifs performants (1 mois)", styles["Heading2"]))
    elements.append(Image(img_top5_buffer, width=6*inch, height=2*inch))
    elements.append(Spacer(1, 0.25*inch))

    # --- Répartition des actifs ---
    plt.figure(figsize=(6, 3))
    plt.pie(weights, labels=[ticker_names.get(t, t) for t in weights.index], autopct="%1.1f%%", startangle=90)
    plt.title("Répartition des actifs")
    img_pie_buffer = BytesIO()
    plt.savefig(img_pie_buffer, format="png", dpi=300, bbox_inches="tight")
    img_pie_buffer.seek(0)
    plt.close()
    elements.append(Paragraph("Répartition des actifs", styles["Heading2"]))
    elements.append(Image(img_pie_buffer, width=4*inch, height=2*inch))
    elements.append(Spacer(1, 0.25*inch))
    ...
    metrics_data = [
        ["Métrique", "Portefeuille", "Portefeuille Hedgé", "Benchmark"],
        ["Perf veille (%)", f"{portfolio_perf_yesterday:.2f}", f"{portfolio_hedged_perf_yesterday:.2f}", f"{benchmark_perf_yesterday:.2f}"],
        ["Perf 1 an (%)", f"{portfolio_perf_1y:.2f}", f"{portfolio_hedged_perf_1y:.2f}", f"{benchmark_perf_1y:.2f}"],
        ["Perf 3 ans (%)", f"{portfolio_perf_3y:.2f}", f"{portfolio_hedged_perf_3y:.2f}", f"{benchmark_perf_3y:.2f}"]
    ]
    ...
    elements.append(Paragraph("Métriques de performance", styles["Heading2"]))
    ...
    elements.append(Paragraph("© 2026 - Institution Financière. Tous droits réservés.", footer_style))
...
# --- Bouton pour générer et télécharger la factsheet ---
if st.button("📥 Générer la Factsheet PDF"):
    # Générer le PDF
    pdf_buffer = generate_pdf_factsheet(
        portfolio_index, portfolio_index_hedged, bench_index,
        weights, top5, prices_eur, ticker_names
    )

    # Préparer le téléchargement
    st.success("Factsheet générée avec succès !")
    st.download_button(
        label="Télécharger la Factsheet PDF",
        data=pdf_buffer,
        file_name="Factsheet_Financière.pdf",
